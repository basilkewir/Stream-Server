...
class FlussonicService
{
    ...
    public function getPlaybackUrls(string $name): array
    {
        ...
        return [
            'rtmp' => "rtmp://{$host}:{$rtmpPort}/static/{$name}",
            'hls' => "http://{$host}/static/{$name}/index.m3u8",
            'dash' => "http://{$host}/static/{$name}/manifest.mpd",
            'screenshot' => "http://{$host}/static/{$name}/screenshot.jpg",
        ];
    }
